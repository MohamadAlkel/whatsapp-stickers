...
import { FormImg, FormImgWrapper, FormImgTitle, FormImgSubtitle, ErrorMessage } from "./MainPage.style";
...
export const PinForm: ConnectedComponent<any, any> = connect(mapStateToProps, mapDispatchToProps)( ({
    isFetchingVerifyPin,
     verifyPin,
      userId
}) => {
  ...
  const onClickConfirm =()=> {
      ...
      verifyPin(pin.join(""), userId).then((data)=>{
        ...
        setErrorMsg("Oops! something went wrong, refresh the page, or try later.");
      }))
  }
  ...
  return (
        <FormImgWrapper>
        <FormImg src={pinForm}/>
        <FormImgTitle position="148px">
          The sticker packs is ready!
        </FormImgTitle>
        <FormImgSubtitle position="179px">
          We’ve sent you a <span>4 digit code </span><br/>please enter it below to confirm subscription!
        </FormImgSubtitle>
        <PinInput onPinChange={onPinChange}/>
        <Button onClick={onClickConfirm} position="284px" isDisabled={true} text="Confirm" moveTo="thanks"/>
